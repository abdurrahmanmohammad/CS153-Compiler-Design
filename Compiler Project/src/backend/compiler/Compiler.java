...
public class Compiler extends SixtyFortranBaseVisitor<Object>
{
    ...
    public Object visitTypeDefinition(SixtyFortranParser.TypeDefinitionContext ctx)
    {
        ...
        {
            routineDefinitionHelper(routineCtx, ctx.type);
        }
    ...
    public Object visitAssignExistingVariable(SixtyFortranParser.AssignExistingVariableContext ctx)
    {
        ...
        //TODO: work on this
        code.emit("invokevirtual " + ctx.call().type.getPath() + "/" + Predefined.OPERATOR_ASSIGN + "(L" + ctx.expression().type.getPath() + ";" + ")V");
        return null;
    }
    ...
    {
        routineDefinitionHelper(ctx, null);
        
        return null;
    }
    
    private void routineDefinitionHelper(SixtyFortranParser.RoutineDefinitionContext ctx, TypeSpec type)
    {
        enterFile(ctx.file, ctx.routineScope);
        
        ScopeEntry routineEntry = localScope.lookupEntry(ctx.routine.name);
        ...
        visitChildren(ctx);
        code.emitRoutineCloser(ctx, ctx.isStatic);
    ...
    public Object visitRoutineCall(SixtyFortranParser.RoutineCallContext ctx)
    {
        ...
        if(entry != null)
        {
            ...
            TypeSpec returnType = routine.returnType;
