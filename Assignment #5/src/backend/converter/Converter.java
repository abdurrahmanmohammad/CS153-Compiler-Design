...
public class Converter extends PascalBaseVisitor<Object> {
	...
	public Object visitProgram(PascalParser.ProgramContext ctx) {
		...
		code.indent();
		code.emitLine("private static java.util.Scanner _sysin = " + "new java.util.Scanner(System.in);");
		code.emitLine();

		// Level 1 declarations.
		PascalParser.ProgramIdentifierContext idCtx = ctx.programHeader().programIdentifier();
	...
	public Object visitConstantDefinition(PascalParser.ConstantDefinitionContext ctx) {
		...
		code.emitEnd("final " + javaTypeName + " " + constantName + " = " + constCtx.getText() + ";");
	...
	public Object visitRecordTypespec(PascalParser.RecordTypespecContext ctx) {
		PascalParser.RecordFieldsContext fieldsCtx = ctx.recordType().recordFields();
		recordFields = true;
	...
	public Object visitVariableDeclarations(PascalParser.VariableDeclarationsContext ctx) {
		...
		for (PascalParser.VariableIdentifierContext varCtx : listCtx.variableIdentifier()) {
			code.emitStart();
			...
			if (typeCtx.type.getForm() == ARRAY)
				emitArraySpecifier(typeCtx.type);
			code.emitEnd(";");
		}
	...
	@Override
	public Object visitVariableIdentifierList(PascalParser.VariableIdentifierListContext ctx) {
		String separator = " ";

		for (PascalParser.VariableIdentifierContext varCtx : ctx.variableIdentifier()) {
			code.emit(separator);
	...
	public Object visitParameterDeclarations(PascalParser.ParameterDeclarationsContext ctx) {
		PascalParser.ParameterIdentifierListContext parmListCtx = ctx.parameterIdentifierList();
		PascalParser.TypeIdentifierContext typeCtx = ctx.typeIdentifier();
		Typespec parmType = typeCtx.type;

		// Loop over the parameters.
		for (PascalParser.ParameterIdentifierContext parmIdCtx : parmListCtx.parameterIdentifier()) {
			...
			if (parmType.getForm() == ARRAY)
				emitArraySpecifier(parmType);
			currentSeparator = ", ";
		}
	 ...
	 */
	private void emitAllocateStructuredData(String lhsPrefix, SymtabEntry variableId) {
	...
	/**
	 * Emit a string of bracketed dimension sizes for the array datatype. followed
	 * by the "new" array allocation.
	 * 
	 * @param type the array datatype.
	 ...
	 */
	private void emitNewArrayElement(String lhsPrefix, String variableName, Typespec elmtType) {
		...
		do {
			...
			code.emitLine(
					"for (int " + subscript + " = 0; " + subscript + " < " + elmtCount + "; " + subscript + "++)");
			code.emitStart("{");
			code.indent();
	 ...
	 */
	private void emitNewRecord(String lhsPrefix, String variableName, Typespec recordType) {
	...
	@Override
	public Object visitCompoundStatement(PascalParser.CompoundStatementContext ctx) {
		code.emit("{");
		code.indent();
		visitChildren(ctx);
		code.dedent();
	...
	public Object visitAssignmentStatement(PascalParser.AssignmentStatementContext ctx) {
		String lhs = (String) visit(ctx.lhs().variable()); // Get rhs
		String expr = (String) visit(ctx.rhs().expression()); // Get lhs
		code.emit(lhs + " = " + expr); // Print assignment
		code.emitEnd(";"); // Append semicolon
		return null;
	}
	...
	public Object visitIfStatement(PascalParser.IfStatementContext ctx) {
		// For an if statement with only 1 expression in true and false blocks
		code.emitStart("if ("); // Print (on new line): if(
		code.emit((String) visit(ctx.expression())); // Print: expression
		code.emitEnd(")"); // Print: )\n
		visitTrueStatement(ctx.trueStatement()); // Print: statement
		if (ctx.ELSE() != null) { // If there is an else statement
			code.emitLine("else"); // Print (on new line): else
			visitFalseStatement(ctx.falseStatement()); // Print (on next line): statement
			// code.emitEnd(""); // Print: \n
		}
		return null;
	}
	...
	@Override
	public Object visitTrueStatement(PascalParser.TrueStatementContext ctx) {
		if (ctx.statement().compoundStatement() != null) { // If compound statement
			code.emitStart(); // Print: \n
			visitStatement(ctx.statement()); // Print: code block
		} else {
			code.indent(); // Start: Indent this block
			code.emitStart(); // Print: \n
			visitStatement(ctx.statement()); // Print: single statement
			code.dedent(); // End: Indent this block
		}
		return null;
	}

	@Override
	public Object visitFalseStatement(PascalParser.FalseStatementContext ctx) {
		if (ctx.statement().compoundStatement() != null) { // If compound statement
			code.emitStart(); // Print: \n
			visitStatement(ctx.statement()); // Print: code block
		} else {
	...
	public Object visitWhileStatement(PascalParser.WhileStatementContext ctx) {
		code.emitStart("while ("); // Print: while (
		code.emit((String) visit(ctx.expression())); // Print: expression
		code.emitEnd(")"); // Print: )\n
		if (ctx.statement().compoundStatement() != null) { // If compound statement
			code.emitStart(); // Print: \n
	...
	public Object visitForStatement(PascalParser.ForStatementContext ctx) {
		// Step 1: Parse: for(int i = 0; i <= x; i++)
		code.emitStart("for ("); // Print: for (
		code.emit((String) visit(ctx.variable())); // Print: i (or other variable)
		code.emit(" = "); // Print: =
		code.emit((String) visit(ctx.expression(0))); // Print: expression
		code.emit("; "); // Print: ;
		code.emit((String) visit(ctx.variable())); // Print: i (or other variable)
		if (ctx.TO() != null)
			code.emit(" <= "); // Print: <=
			...
			code.emit(" >= "); // Print: >=
		code.emit((String) visit(ctx.expression(1))); // Print: expression
		code.emit("; "); // Print: ;
		code.emit((String) visit(ctx.variable())); // Print: i (or other variable)
		if (ctx.TO() != null)
			code.emit("++"); // Print: ++
		...
		// Step 2: Print statements in for loop
		if (ctx.statement().compoundStatement() != null) { // If compound statement
			code.emitStart(); // Print: \n
			visitStatement(ctx.statement()); // Print: code block
		} else {
	...
	public Object visitCaseStatement(PascalParser.CaseStatementContext ctx) {
		code.emitStart("switch("); // Print: switch(
		code.emit((String) visit(ctx.expression())); // Print: expression
		code.emitEnd(")"); // Print: )
		code.emitLine("{"); // Print: {
		code.indent(); // Start: Indent block
		visit(ctx.caseBranchList()); // Print case statements
		code.dedent(); // Stop: Indent block
		code.emitLine("}"); // Print: }
		return null;
	}
	...
	public Object visitCaseBranch(PascalParser.CaseBranchContext ctx) {
		// A case branch could be empty, so we need to check if the constant list exists
		if (ctx.caseConstantList() != null) { // Could also check if the statement exists
			visit(ctx.caseConstantList());
			visit(ctx.statement());
			// Most statements include newline, so break statement will be on the next line
			// This is why I intended the break statement
			code.indent(); // Start: Indent block
			code.emitStart("break;"); // Print: break
			code.dedent(); // Stop: Indent block
		}
		return null;
	}
	...
	public Object visitCaseConstantList(PascalParser.CaseConstantListContext ctx) {
		for (int i = 0; i < ctx.caseConstant().size(); i++) // If no statements, don't print
			code.emitStart("case " + ctx.caseConstant(i).getText() + ": "); // Print: case 1 :
		return null;
	}
	// ######################### Stop: Case Statement #########################
	...
	public Object visitProcedureCallStatement(PascalParser.ProcedureCallStatementContext ctx) {
		// For an if statement with only 1 expression in true and false blocks
		code.emitStart(ctx.procedureName().getText()); // Print (on new line): procedure(
		code.emit("("); // Print: (
		for (int i = 0; ctx.argumentList() != null && i < ctx.argumentList().argument().size(); i++) {
			code.emit(ctx.argumentList().argument().get(i).getText()); // Print: a, b, c, ...
			if (i < ctx.argumentList().argument().size() - 1)
				code.emit(","); // Don't add , on last param
		}
		code.emitEnd(");"); // Print: )\n
		return null;
	}
	...
	public Object visitVariable(PascalParser.VariableContext ctx) {
		...
		if ((type != Predefined.booleanType) && (variableId.getKind() == ENUMERATION_CONSTANT)) {
		...
		for (PascalParser.ModifierContext modCtx : ctx.modifier()) {
			...
			if (modCtx.indexList() != null) {
				for (PascalParser.IndexContext indexCtx : modCtx.indexList().index()) {
					...
					String subscript = (minIndex == 0) ? expr
							: (minIndex < 0) ? "(" + expr + ")+" + (-minIndex) : "(" + expr + ")-" + minIndex;
		...
		for (int i = 0; i < size; i++) {
			...
			// Read any other value.
				...
				String typeName = type == Predefined.integerType ? "Int"
						: type == Predefined.realType ? "Double" : type == Predefined.booleanType ? "Boolean" : "";
